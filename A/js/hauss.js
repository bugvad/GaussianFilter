...
img.onload = function() { //как только картинка подгрузится запускается функция
    ...
    var frag_shader = gl.createShader(gl.FRAGMENT_SHADER); //A Fragment Shader is the Shader stage that will process a Fragment generated by the Rasterization into a set of colors and a single depth value.
    ...
    gl.shaderSource(vtx_shader, vtx_shader_source); //method of the WebGL API sets the source code of a WebGLShader.
    gl.shaderSource(frag_shader, frag_shader_source); 
    gl.compileShader(vtx_shader); //компилирует шейдер GLSL в двоичные данные, чтобы он мог использоваться программой WebGLProgram
